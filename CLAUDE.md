...
| `content` | **Repaso informativo (HTML):** `title` opcional + `html` (multilínea `"""..."""`) que se renderiza como mini-página (encabezados, colores, estilos inline, layout). **Solo lectura**, sin input. El front lo sanea con **DOMPurify** (bloquea `<script>`/`onclick`/`javascript:`). Sirve para explicar el tema antes de la evaluación. | No se califica (excluido del score) | OK |
...
- **`content` (repaso HTML, no calificable):** bloque informativo de solo lectura. DSL `content { title html: """<html>""" }`; renderer `ContentRenderer` saneado con **DOMPurify** (`.rich-content` en app.css restaura tipografía de encabezados/listas que Tailwind resetea). Excluido de la calificación (`_build_answer_details` hace `continue`) y del chrome "Actividad N/Interactiva" del `WorksheetRenderer`. Sí se imprime (`WorksheetPrint`). Editor visual con vista previa en vivo. Nota parser: el DSL cuenta llaves `{}` para delimitar bloques, así que el HTML/CSS debe tener llaves balanceadas (todo HTML válido lo está); una llave suelta en texto rompería el parseo → usar `&#123;`/`&#125;`.
