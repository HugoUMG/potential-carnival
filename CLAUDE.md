...
- **`content` (repaso HTML, no calificable):** bloque informativo de solo lectura. DSL `content { title html: """<html>""" }`. **Dos modos de render:** (a) por defecto, saneado inline con **DOMPurify** (`ContentRenderer`→`InlineContent`; `.rich-content` en app.css restaura tipografía que Tailwind resetea; se integra con el tema y se imprime); (b) `sandbox: true` → `SandboxedHtml.tsx` renderiza el HTML **completo** en un `<iframe sandbox="allow-scripts">` (SIN `allow-same-origin`): permite CSS/JS/fuentes propios, aislado de la app (no toca DOM/cookies/localStorage del portal). Auto-altura vía `postMessage` (se inyecta un snippet en el srcDoc; el padre valida `e.source`). En impresión siempre se usa la versión saneada estática. Excluido de la calificación (`_build_answer_details` hace `continue`) y del chrome "Actividad N/Interactiva". Editor visual con toggle "Página completa" + vista previa en vivo.
